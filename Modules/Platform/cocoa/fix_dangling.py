...
# Wait, protocols have `var foo: Int { get set }` on one line, which is fine.
# Our orphaned ones are on their own lines.
...
while i < len(lines):
    ...
    if stripped in ["get", "set", "get set"]:
        ...
        if not prev_stripped.endswith("{"):
            ...
            # But there could be `set` after `get`.
            # Let's scan forward.
            j = i + 1
            while j < len(lines):
...
print(f"Fixed {len(lines_to_delete)} dangling accessors/braces.")
